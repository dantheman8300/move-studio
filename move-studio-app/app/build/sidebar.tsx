...
export default function Sidebar(props: {
  ...
  const [suiGptPrompt, setSuiGptPrompt] = useState("");
      ...
      toast({
        description: (
          <div className="flex flex-row gap-2 items-center justify-start">
            ...
            <a
              href={`https://suiexplorer.com/txblock/${publishTxn.digest}`}
              target="_blank"
            >
  ...
  const annotatePackage = async () => {
    let file = (currentProject?.files.find((file) => file.name === "sources") as any)?.children[0] as IFile;
    console.log(file);

    console.log('annotating package...')

    let res = await fetch(
      "https://move-annotate-backend.gptutor.tools/api/v1/annotate-move", 
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    console.log('finished annotating package')
                ...
                <Tooltip>
                  <TooltipTrigger className="ps-4 py-2">
                    <div className="flex flex-col items-start gap-1.5">
                      ...
                      <div className="flex flex-row items-center justify-center gap-1">
                        <Input
                          className="w-full max-w-[175px] min-w-[50px] bg-slate-900 h-8 focus-visible:ring-1 focus-visible:ring-ring focus-visible:ring-teal-500 font-mono caret-teal-50 bg-slate-900"
                          type="text"
                          placeholder="0x000..000"
                          value={addedObject}
                          onChange={(e) => {
          ...
          <AccordionContent>
            <div className="flex flex-col items-start gap-1">
              <div className="w-full">
                <Textarea 
                  placeholder="Write Sui GPT prompt here..." 
                  className="focus-visible:ring-1 focus-visible:ring-ring focus-visible:ring-teal-500 font-mono caret-teal-500"
                  value={suiGptPrompt}
                  onChange={(e) => {
                    setSuiGptPrompt(e.target.value);
                  }}
                />
                <Button 
                  variant="ghost" 
